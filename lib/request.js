...
exports.request = function request(path, options, callback) {
...
function expectedStatus(res, options) {
  return (options.expectedStatus && (res.statusCode === options.expectedStatus)) ||
    (!options.expectedStatus && res.statusCode.toString().match(/^2\d{2}$/))
}
...
function handleFailure(res, buffer, options, deferred, callback) {
  ...
  if (options.expectedStatus) {
    message = 'Expected response ' + options.expectedStatus + ', got ' + res.statusCode
  } else {
